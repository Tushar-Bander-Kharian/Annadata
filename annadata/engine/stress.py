...
def nutrient_factor(
    ...
    n_optimal = base_yield_t_ha * n_demand_kg_per_t
    p_optimal = base_yield_t_ha * p_demand_kg_per_t
    k_optimal = base_yield_t_ha * k_demand_kg_per_t

    f_n = min(1.0, n_kg_ha / max(n_optimal, 1.0))
    f_p = min(1.0, p_kg_ha / max(p_optimal, 1.0))
    f_k = min(1.0, k_kg_ha / max(k_optimal, 1.0))
